...
local approval_model = require("app.model.approval")
...
local local_emp_photo_dir = config.local_emp_photo_config.dir
local local_emp_photo_path = config.local_emp_photo_config.path
...
employee_router:post("/:emp_no/avatar/upload",function(req,res,next)
    ...
    if file.success and file.filename then 
	   ...
	   local co = coroutine.create(function (file,emp_no)
	      ...
	      local result, err = os.execute("mv -f "..filepath_temp.."/"..filename 
	      	         .." ".. local_emp_photo_dir.."/"..emp_no..".".. extname)
...
--根据工号找user
employee_router:get("/:emp_no/user",function(req,res,next)
	local emp_no = req.params.emp_no
	...
	local emp_no = supper(emp_no)
	local user_info ,err = emp_model:query_full_empinfo_by_emp(emp_no)
	if not user_info or err then 
		user_info,err = approval_model:query_approval_person_by_empno(emp_no)
		if not user_info or err then 
			return res:json({
				rv = 510,
				msg = "没有工号对应用户记录"
	...
	local no = user_info.no or user_info.emp_no
	local name = user_info.name or user_info.emp_name
...
employee_router:get("/dept_name",function(req,res,next)
	...
	local dept_info ,err = dept_model:query_dept_exists_by_dept(dept_code)
	if not dept_info or err then 
		return res:json({
			rv = 511,
			msg = "没有部门对应信息."
		})
